...
int main()
{
	...
	*/
	...
	/*
	do while 문 : 반복문의 한 종 류
	형태 : do{}whilde(조건식);
	while문은 처음부터 조건식을 체크하지만 do while은 처음 한번은 무조건 동작되고
	그 후 조건식을 체크해서 true일 경우 동작 되는 반복문이다
	*/
	...
	[ 0  | 1  | 2  | 3  |  4 | 5  | 6  |  7  ]
	배열은 index를 이용해서 원하는 부분에 접근하여 값을 지정하 수 있다.
	인덱스는  0부터 개수 까지이다 . 즉, 10개라면  0 ~ 9 까지의 총 10개의 인덱스를 가지게 된다
	*/

	//배열뿐만 아니라 일반 변수들도 선언을 하고 값을 초기화하지 않을 경우 잉여값이 들어가게된다

	
	//int iArray[10] = {1, 2, 3, 4, 5, 6, 7, 8, 9}; || 배열 초기화
	
	/*
	0 ~4번 까지는 1, 2, 3, 4, 5 의 값이 들어가게 되고 나머지는 모두 0 으로 초기화 된다
	...
	*/
	...
	/*
	배열 개수를 2개를 지정하면 이차원배열이 된다. 3개를 지정하면 3차원 배열이 된다.
	2차원 배열의 개수는 앞의수 * 뒤의수 만큼 처리가 된다. 아래는 10 * 10 이 되는 것이다.
	*/
	int IArray2[10][10] = { {1,2,3},{4,5,6} }; //앞 10의 123번 제외하고 나머지는 0 || 뒤 10의 4,5,6,제외하고 나머지 0
	for (int i = 0; i < 10; ++i)
	{
		for (int j = 0; j < 10; ++j)
		{
